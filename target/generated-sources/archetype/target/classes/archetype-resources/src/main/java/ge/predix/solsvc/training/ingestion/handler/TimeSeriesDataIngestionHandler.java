...
import ${package}.ge.predix.solsvc.bootstrap.ams.dto.AssetMeter;
import ${package}.ge.predix.solsvc.bootstrap.tbs.entity.InjectionBody;
import ${package}.ge.predix.solsvc.bootstrap.tbs.entity.InjectionMetric;
import ${package}.ge.predix.solsvc.bootstrap.tbs.entity.InjectionMetricBuilder;
import ${package}.ge.predix.solsvc.bootstrap.tsb.client.TimeseriesWSConfig;
import ${package}.ge.predix.solsvc.bootstrap.tsb.factories.TimeseriesFactory;
import ${package}.ge.predix.solsvc.training.ingestion.HttpWrapper;
import ${package}.ge.predix.solsvc.training.ingestion.api.Constants;
import ${package}.ge.predix.solsvc.training.ingestion.type.CargoShelfLife;
import ${package}.ge.predix.solsvc.training.ingestion.type.Lo${package}otiveGatewayType;
import ${package}.ge.predix.solsvc.training.ingestion.websocket.WebSocketClient;
import ${package}.ge.predix.solsvc.training.ingestion.websocket.WebSocketConfig;
import ${package}.google.gson.Gson;
...
public class TimeSeriesDataIngestionHandler extends BaseFactoryIT
{
    ...
    {
        log.info("*******************TimeSeriesDataIngestionHandler Initialization ${package}plete*********************");
    }
    ...
    public void handleData(String tenentId, String controllerId, String data, String authorization) 
    {
        ...
        if (StringUtils.isEmpty(authorization)) {
        	log.info("Refreshing token locally =====================>>> > token should be ${package}ing from machine --> post MVP !!! :( !!!");
        ...
        try
        {
        	...
        	Lo${package}otiveGatewayType json = new Gson().fromJson(data, Lo${package}otiveGatewayType.class);
    ...
    public InjectionMetricBuilder latBuilder(Lo${package}otiveGatewayType data){
        ...
        InjectionBody body = new InjectionBody( data.getLocdata().getName()); // this should ${package}e from asset services once We are able to integrate the same
    ...
    public InjectionMetricBuilder longBuilder(Lo${package}otiveGatewayType data){
    ...
    public InjectionMetricBuilder rpmBuilder(Lo${package}otiveGatewayType data){
    	Long rpm_Message_number=new Long(System.currentTimeMillis());
    	InjectionMetricBuilder builder = InjectionMetricBuilder.getInstance();
        InjectionMetric metric = new InjectionMetric(rpm_Message_number);
        InjectionBody body = new InjectionBody( data.getRpmData().getName()); // this should ${package}e from asset services once We are able to integrate the same
    ...
    public InjectionMetricBuilder torqueBuilder(Lo${package}otiveGatewayType data){
        ...
        InjectionBody body = new InjectionBody( data.getTorquedata().getName()); // this should ${package}e from asset services once We are able to integrate the same
